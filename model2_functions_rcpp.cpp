...
// [[Rcpp::export]]
mat copy_c(mat lambda_i, mat lambda_small, vec matching, vec treatment,vec allocation_l, int n, int m){
  for (int i=0;i<n;i++){
    for (int t=0;t<m;t++){
      lambda_i(i,t)=lambda_small(matching[i]-1,t)*treatment[allocation_l[i]-1];
    }
  }
  return lambda_i;
}
