...
Rcpp::sourceCpp("model2_functions_rcpp.cpp")
...
lambda_matrix=function(current, data){
  ...
  for (i in 1:length(repre))
  {
    ...
    lambda_small[i,1]=l_unit*(current$b1+current$b2+current$b3)+l_floor*(current$b1+current$b2)+l_block*current$b1
  }
  ...
  for (t in 2:ncol(lambda_small))
  {
    ...
    if (length(which(data$t_inf==current$inf_order[t]))>0)
    {
      ...
      lambda_small[which(distance_repre[ii,]==3),t]=lambda_small[which(distance_repre[ii,]==3),t]+(current$b1+current$b2+current$b3)
      lambda_small[which(distance_repre[ii,]==2),t]=lambda_small[which(distance_repre[ii,]==2),t]+(current$b1+current$b2)
      lambda_small[which(distance_repre[ii,]==1),t]=lambda_small[which(distance_repre[ii,]==1),t]+current$b1
    }
  ...
  lambda_i=copy_c(lambda_i, lambda_small, matching,current$t, allocation_l,nrow(lambda_i), ncol(lambda_i))
}
...
likelihood_contr=function(data, k){
  ...
  lambda_new=lambda_new_cal(lambda_new, distance, inf_list, current$b1*current$t[allocation_l[k]], current$b2*current$t[allocation_l[k]], current$b3*current$t[allocation_l[k]], k, length(lambda_new))
  ...
  if(data$t_inf[k]>old_data$t_inf[k]){
    for(i in which(distance[,k]==3&data$t_inf>=inf_old))
    {
      ...
      if(i==k){
        ...
        data$lambda_inf[k]=(current$b1*l_block+(current$b1+current$b2)*l_floor+(current$b1+current$b2+current$b3)*l_unit)*current$t[allocation_l[k]]
        ...
        if (data$T0[i]-139>inf_old) delta=delta+(current$b1+current$b2+current$b3)*(min(data$T0[i]-139,inf_new)-inf_old)*current$t[allocation_l[i]]
        if (data$T0[i]-139>rem_old) delta=delta-(current$b1+current$b2+current$b3)*((data$T0[i]-139)-rem_old)*current$t[allocation_l[i]]
      }else if(data$S1[i]==0&data$t_inf[i]>end){
        if (rem_new<data$T1[i]-139) next
        if (data$T1[i]-139>inf_old) delta=delta+(current$b1+current$b2+current$b3)*(min(data$T1[i]-139,inf_new)-inf_old)*current$t[allocation_l[i]]
        if (data$T1[i]-139>rem_old) delta=delta-(current$b1+current$b2+current$b3)*((data$T1[i]-139)-rem_old)*current$t[allocation_l[i]]
      }else if(data$t_inf[i]<=inf_new){
          data$lambda_inf[i]=old_data$lambda_inf[i]-(current$b1+current$b2+current$b3)*current$t[allocation_l[i]]
          if (data$lambda_inf[i]<=0){
            delta=-999999
            data$delta=delta
            return(data)
          }else{
            delta=delta-log(old_data$lambda_inf[i])+log(data$lambda_inf[i])+(current$b1+current$b2+current$b3)*(data$t_inf[i]-inf_old)*current$t[allocation_l[i]]
          }
        ...
        if (data$lambda_inf[i]<=0){
          ...
          delta=delta-log(old_data$lambda_inf[i])+log(data$lambda_inf[i])+(current$b1+current$b2+current$b3)*(rem_new-data$t_inf[i])*current$t[allocation_l[i]]
        }
      }else if (data$t_inf[i]>inf_new&data$t_inf[i]<=rem_old){
        delta=delta+(current$b1+current$b2+current$b3)*(inf_new-inf_old)*current$t[allocation_l[i]]
      }
    }
    for(i in which(distance[,k]==2&data$t_inf>=inf_old))
    {
      ...
      if (data$S1[i]==2&data$S0[i]==0&data$t_inf[i]>end){
        if (data$T0[i]-139>rem_new) next
        if (data$T0[i]-139>inf_old) delta=delta+(current$b1+current$b2)*(min(data$T0[i]-139,inf_new)-inf_old)*current$t[allocation_l[i]]
        if (data$T0[i]-139>rem_old) delta=delta-(current$b1+current$b2)*((data$T0[i]-139)-rem_old)*current$t[allocation_l[i]]
      }else if(data$S1[i]==0&data$t_inf[i]>end){
        if (data$T1[i]-139>rem_new) next
        if (data$T1[i]-139>inf_old) delta=delta+(current$b1+current$b2)*(min(data$T1[i]-139,inf_new)-inf_old)*current$t[allocation_l[i]]
        if (data$T1[i]-139>rem_old) delta=delta-(current$b1+current$b2)*((data$T1[i]-139)-rem_old)*current$t[allocation_l[i]]        
      }else if (data$t_inf[i]<=inf_new){
        delta=delta-log(old_data$lambda_inf[i])
        data$lambda_inf[i]=old_data$lambda_inf[i]-(current$b1+current$b2)*current$t[allocation_l[i]]
        if (data$lambda_inf[i]<=0){
          delta=-999999
          data$delta=delta
          return(data)
        }else{
          delta=delta+log(data$lambda_inf[i])+(current$b1+current$b2)*(data$t_inf[i]-inf_old)*current$t[allocation_l[i]]
        }
      }else if(data$t_inf[i]>rem_old&data$t_inf[i]<=rem_new){
        data$lambda_inf[i]=old_data$lambda_inf[i]+(current$b1+current$b2)*current$t[allocation_l[i]]
        if (data$lambda_inf[i]<=0){
          ...
          delta=delta-log(old_data$lambda_inf[i])+log(data$lambda_inf[i])+(current$b1+current$b2)*(rem_new-data$t_inf[i])*current$t[allocation_l[i]]
        }
      }else if (data$t_inf[i]>inf_new&data$t_inf[i]<=rem_old){
        delta=delta+(current$b1+current$b2)*(inf_new-inf_old)*current$t[allocation_l[i]]
      }
    }
    for(i in which(distance[,k]==1&data$t_inf>inf_old))
    {
      ...
      if (data$S1[i]==2&data$S0[i]==0&data$t_inf[i]>end){
        if (data$T0[i]-139>rem_new) next
        if (data$T0[i]-139>inf_old) delta=delta+(current$b1)*(min(data$T0[i]-139,inf_new)-inf_old)*current$t[allocation_l[i]]
        if (data$T0[i]-139>rem_old) delta=delta-(current$b1)*((data$T0[i]-139)-rem_old)*current$t[allocation_l[i]]
      }else if(data$S1[i]==0&data$t_inf[i]>end){
        if (data$T1[i]-139>rem_new) next
        if (data$T1[i]-139>inf_old) delta=delta+(current$b1)*(min(data$T1[i]-139,inf_new)-inf_old)*current$t[allocation_l[i]]
        if (data$T1[i]-139>rem_old) delta=delta-(current$b1)*((data$T1[i]-139)-rem_old)*current$t[allocation_l[i]]     
      }else if(data$t_inf[i]<=inf_new){
        delta=delta-log(old_data$lambda_inf[i])
        data$lambda_inf[i]=old_data$lambda_inf[i]-(current$b1)*current$t[allocation_l[i]]
        if (data$lambda_inf[i]<=0){
          delta=-999999
          data$delta=delta
          return(data)
        }else{
          delta=delta+log(data$lambda_inf[i])+(current$b1)*(data$t_inf[i]-inf_old)*current$t[allocation_l[i]]
        }
      }else if(data$t_inf[i]>rem_old&data$t_inf[i]<=rem_new){
        data$lambda_inf[i]=old_data$lambda_inf[i]+(current$b1)*current$t[allocation_l[i]]
        if (data$lambda_inf[i]<=0){
          ...
          delta=delta-log(old_data$lambda_inf[i])+log(data$lambda_inf[i])+(current$b1)*(rem_new-data$t_inf[i])*current$t[allocation_l[i]]
        }
      }else if (data$t_inf[i]>inf_new&data$t_inf[i]<=rem_old){
        delta=delta+(current$b1)*(inf_new-inf_old)*current$t[allocation_l[i]]
      }
    }
  }
  if(data$t_inf[k]<old_data$t_inf[k]){
    for(i in which(distance[,k]==3&data$t_inf>=inf_new))
    {
      ...
      if(i==k){
        ...
        if (data$T0[i]-139>inf_new) delta=delta-(current$b1+current$b2+current$b3)*(min(data$T0[i]-139,inf_old)-inf_new)*current$t[allocation_l[i]]
        if (data$T0[i]-139>rem_new) delta=delta+(current$b1+current$b2+current$b3)*((data$T0[i]-139)-rem_new)*current$t[allocation_l[i]]
      }else if(data$S1[i]==0&data$t_inf[i]>end){
        if (rem_old<data$T1[i]-139) next
        if (data$T1[i]-139>inf_new) delta=delta-(current$b1+current$b2+current$b3)*(min(data$T1[i]-139,inf_old)-inf_new)*current$t[allocation_l[i]]
        if (data$T1[i]-139>rem_new) delta=delta+(current$b1+current$b2+current$b3)*((data$T1[i]-139)-rem_new)*current$t[allocation_l[i]]
          ...
          if (data$lambda_inf[i]<=0){
            ...
            delta=delta-log(old_data$lambda_inf[i])+log(data$lambda_inf[i])-(current$b1+current$b2+current$b3)*(data$t_inf[i]-inf_new)*current$t[allocation_l[i]]
          }
        ...
        if (data$lambda_inf[i]<=0){
          ...
          delta=delta-log(old_data$lambda_inf[i])+log(data$lambda_inf[i])-(current$b1+current$b2+current$b3)*(rem_old-data$t_inf[i])*current$t[allocation_l[i]]
        }
      }else if (data$t_inf[i]>inf_old&data$t_inf[i]<=rem_new){
        delta=delta-(current$b1+current$b2+current$b3)*(inf_old-inf_new)*current$t[allocation_l[i]]
      }
    }
    for(i in which(distance[,k]==2&data$t_inf>inf_new))
    {
      ...
      if (data$S1[i]==2&data$S0[i]==0&data$t_inf[i]>end){
        if (data$T0[i]-139>rem_old) next
        if (data$T0[i]-139>inf_new) delta=delta-(current$b1+current$b2)*(min(data$T0[i]-139,inf_old)-inf_new)*current$t[allocation_l[i]]
        if (data$T0[i]-139>rem_new) delta=delta+(current$b1+current$b2)*((data$T0[i]-139)-rem_new)*current$t[allocation_l[i]]
      }else if(data$S1[i]==0&data$t_inf[i]>end){
        if (rem_old<data$T1[i]-139) next
        if (data$T1[i]-139>inf_new) delta=delta-(current$b1+current$b2)*(min(data$T1[i]-139,inf_old)-inf_new)*current$t[allocation_l[i]]
        if (data$T1[i]-139>rem_new) delta=delta+(current$b1+current$b2)*((data$T1[i]-139)-rem_new)*current$t[allocation_l[i]]
      }else if(data$t_inf[i]<=inf_old){
        data$lambda_inf[i]=old_data$lambda_inf[i]+(current$b1+current$b2)*current$t[allocation_l[i]]
        if (data$lambda_inf[i]<=0){
          delta=-999999
          ...
          delta=delta-log(old_data$lambda_inf[i])+log(data$lambda_inf[i])-(current$b1+current$b2)*(data$t_inf[i]-inf_new)*current$t[allocation_l[i]]
        }
      }else if(data$t_inf[i]>rem_new&data$t_inf[i]<=rem_old){
        data$lambda_inf[i]=old_data$lambda_inf[i]-(current$b1+current$b2)*current$t[allocation_l[i]]
        if (data$lambda_inf[i]<=0){
          ...
          delta=delta-log(old_data$lambda_inf[i])+log(data$lambda_inf[i])-(current$b1+current$b2)*(rem_old-data$t_inf[i])*current$t[allocation_l[i]]
        }
      }else if (data$t_inf[i]>inf_old&data$t_inf[i]<=rem_new){
        delta=delta-(current$b1+current$b2)*(inf_old-inf_new)*current$t[allocation_l[i]]
      }
    }
    for(i in which(distance[,k]==1&data$t_inf>inf_new))
    {
      ...
      if (data$S1[i]==2&data$S0[i]==0&data$t_inf[i]>end){
        if (data$T0[i]-139>rem_old) next
        if (data$T0[i]-139>inf_new) delta=delta-(current$b1)*(min(data$T0[i]-139,inf_old)-inf_new)*current$t[allocation_l[i]]
        if (data$T0[i]-139>rem_new) delta=delta+(current$b1)*((data$T0[i]-139)-rem_new)*current$t[allocation_l[i]]
      }else if(data$S1[i]==0&data$t_inf[i]>end){
        if (rem_old<data$T1[i]-139) next
        if (data$T1[i]-139>inf_new) delta=delta-(current$b1)*(min(data$T1[i]-139,inf_old)-inf_new)*current$t[allocation_l[i]]
        if (data$T1[i]-139>rem_new) delta=delta+(current$b1)*((data$T1[i]-139)-rem_new)*current$t[allocation_l[i]]
      }else if(data$t_inf[i]<=inf_old){
        data$lambda_inf[i]=old_data$lambda_inf[i]+(current$b1)*current$t[allocation_l[i]]
        if (data$lambda_inf[i]<=0){
          delta=-999999
          ...
          delta=delta-log(old_data$lambda_inf[i])+log(data$lambda_inf[i])-(current$b1)*(data$t_inf[i]-inf_new)*current$t[allocation_l[i]]
        }
      }else if(data$t_inf[i]>rem_new&data$t_inf[i]<=rem_old){
        data$lambda_inf[i]=old_data$lambda_inf[i]-(current$b1)*current$t[allocation_l[i]]
        if (data$lambda_inf[i]<=0){
          ...
          delta=delta-log(old_data$lambda_inf[i])+log(data$lambda_inf[i])-(current$b1)*(rem_old-data$t_inf[i])*current$t[allocation_l[i]]
        }
      }else if (data$t_inf[i]>inf_old&data$t_inf[i]<=rem_new){
        delta=delta-(current$b1)*(inf_old-inf_new)*current$t[allocation_l[i]]
      }
    }
  }
  data$delta=delta
